...
class TableManager(object):
    ...
    def request_plot_object(self, use_axes):
...
class HistoryManager(object):
    ...
    def add_plot_obj(self, new_obj):
        '''adds a new plot obj'''
        self._history = np.insert(self._history, 0, new_obj)
    def get_obj(self, orig, text):
        '''checks to see if any of the objects in history have orig and text.
...
class GraphManager(object):
    ...
    def __init__(self, history_manager, table_manager):
        '''history is a HistoryManager'''
        self._history = history_manager
        self._table_manager = table_manager
    def graph_it(self, orig_txt_lst):
        '''gets passed a list of tuples containing pts and txt.  graphs them'''
        plots = []
        for orig, text in orig_txt_lst:
            to_plot = self._history.get_obj(orig, text)
            if not to_plot:
                to_plot = self._table_manager.request_plot_object()
